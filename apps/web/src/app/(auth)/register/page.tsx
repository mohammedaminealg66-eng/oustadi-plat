...
import { Button, Input, Card, CardContent, CardHeader, cn } from '@oustadi/ui';

import { GraduationCap, ArrowLeft, UserCircle, Briefcase } from 'lucide-react';
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="border-none shadow-2xl shadow-gray-900/5 rounded-[2.5rem] overflow-hidden bg-white dark:bg-gray-800">
          <CardHeader className="pt-8 sm:pt-10 pb-4 sm:pb-6 text-center border-none">
            <h1 className="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight dark:text-gray-100">{t('createAccount')}</h1>
            <p className="mt-1 sm:mt-2 text-[11px] sm:text-sm font-bold text-gray-400 uppercase tracking-widest dark:text-gray-500">{t('joinPlatform')}</p>
          </CardHeader>
          <CardContent className="px-5 sm:px-8 pb-8 sm:pb-10">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                <Input id="fullName" label={t('fullName')} placeholder="Jhon Doe" value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} required />
                <Input id="email" label={t('email')} type="email" placeholder="name@example.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required />
                <Input id="password" label={t('password')} type="password" placeholder="••••••••" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required />
                
                <div className="space-y-2">
                  <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1 dark:text-gray-500">{t('role')}</label>
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      type="button"
                      onClick={() => setForm({ ...form, role: 'STUDENT' })}
                      className={cn(
                        "flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all active:scale-[0.98]",
                        form.role === 'STUDENT' 
                          ? "bg-primary-50 border-primary-600 text-primary-700 shadow-sm" 
                          : "bg-white border-gray-100 text-gray-400 hover:border-gray-200"
                      )}
                    >
                      <UserCircle className={cn("h-6 w-6", form.role === 'STUDENT' ? "text-primary-600 dark:text-primary-400" : "text-gray-300 dark:text-gray-500")} />
                      <span className="text-xs font-black uppercase tracking-tight">{t('student')}</span>
                    </button>
                    <button
                      type="button"
                      ...
                      className={cn(
                        "flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all active:scale-[0.98]",
                        form.role === 'TEACHER' 
                          ? "bg-primary-50 border-primary-600 text-primary-700 shadow-sm dark:bg-primary-900/20 dark:text-primary-400" 
                          : "bg-white border-gray-100 text-gray-400 hover:border-gray-200 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:border-gray-600"
                      )}
                    >
                      <Briefcase className={cn("h-6 w-6", form.role === 'TEACHER' ? "text-primary-600 dark:text-primary-400" : "text-gray-300 dark:text-gray-500")} />
                      <span className="text-xs font-black uppercase tracking-tight">{t('teacher')}</span>
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-10 text-center">
              <p className="text-sm font-bold text-gray-400 dark:text-gray-500">
                {t('hasAccount')} <Link href="/login" className="font-black text-primary-600 hover:text-primary-700 underline decoration-primary-200 underline-offset-4 decoration-2 dark:text-primary-400 dark:hover:text-primary-500 dark:decoration-primary-700">{c('login')}</Link>
              </p>
            </div>
          </CardContent>
        </Card>
